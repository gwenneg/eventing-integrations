...
@QuarkusTest
@TestProfile(SplunkIntegrationInsecureSchemeTest.class)
public class SplunkIntegrationInsecureSchemeTest extends CamelQuarkusTestSupport {
    /**
     ...
     */
    @Test
    void testInvalidTargetUrlScheme() throws Exception {
        ...
        // Update the "target url" field and set an invalid "http" scheme,
        // which should be rejected by the "TargetUrlValidator".
        final JsonObject notifMetadata = data.getJsonObject(CloudEventTestHelper.FIELD_NOTIF_METADATA);
        notifMetadata.put(CloudEventTestHelper.FIELD_NOTIF_METADATA_URL, "http://example.com");

        final Exchange invalidSchemeExchange = createExchangeWithBody(cloudEvent.toString());
        ...
        cloudEventDecoder.process(invalidSchemeExchange);
